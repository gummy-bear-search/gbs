...
async fn test_index_document_overwrite() {
    ...
    assert_eq!(
        source.get("title").and_then(|t| t.as_str()).unwrap(),
        "Updated"
    );
...
async fn test_document_with_various_types() {
    ...
    assert_eq!(
        source.get("string").and_then(|s| s.as_str()).unwrap(),
        "value"
    );
    assert_eq!(source.get("number").and_then(|n| n.as_u64()).unwrap(), 42);
    assert_eq!(
        source.get("boolean").and_then(|b| b.as_bool()).unwrap(),
        true
    );
...
async fn test_match_indices_wildcard() {
    ...
    storage
        .create_index("test_index_1", None, None)
        .await
        .unwrap();
    storage
        .create_index("test_index_2", None, None)
        .await
        .unwrap();
    storage
        .create_index("other_index", None, None)
        .await
        .unwrap();
...
async fn test_search_nonexistent_index() {
    ...
    let query = serde_json::json!({ "match_all": {} });
    let result = storage
        .search("nonexistent", &query, None, None, None, None, None)
        .await;
...
async fn test_search_pagination_beyond_results() {
    ...
    let result = storage
        .search("test_index", &query, Some(10), Some(10), None, None, None)
        .await
        .unwrap();
    let hits = result
        .get("hits")
...
async fn test_search_pagination_zero_size() {
    ...
    // Request with size 0
    let result = storage
        .search("test_index", &query, Some(0), Some(0), None, None, None)
        .await
        .unwrap();
    let hits = result
        .get("hits")
...
async fn test_search_range_boundary_values() {
    ...
    storage
        ...
        .unwrap();
    storage
        .index_document("test_index", "1", serde_json::json!({"age": 25}))
        .await
        .unwrap();
    storage
        .index_document("test_index", "2", serde_json::json!({"age": 30}))
        .await
        .unwrap();
    storage
        .index_document("test_index", "3", serde_json::json!({"age": 35}))
        .await
        .unwrap();
...
async fn test_search_bool_empty_clauses() {
    ...
    storage
        ...
        .unwrap();
    storage
        .index_document("test_index", "1", serde_json::json!({"title": "Test"}))
        .await
        .unwrap();
...
async fn test_bulk_update_merge() {
    ...
    assert_eq!(source.get("count").and_then(|c| c.as_u64()).unwrap(), 2);
    assert_eq!(
        source.get("new_field").and_then(|f| f.as_str()).unwrap(),
        "value"
    );
}
